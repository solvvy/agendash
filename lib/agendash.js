...
module.exports = function(agenda, options) {
  ...
  function requeueJobs(jobIds, callback) {
    ...
    const ids = jobIds.map(jobId => agenda._collection.s.pkFactory(jobId));

    return Bluebird.map(ids, id =>
      agenda._collection.findOneAndUpdate({ _id: id }, { $set: { nextRunAt: new Date() } })
    )
      .then(jobs => {
  ...
  function deleteJobs(jobIds, callback) {
    ...
    try {
      agenda.cancel(
        {
          _id: {
            $in: jobIds.map(jobId => agenda._collection.s.pkFactory(jobId))
          }
        },
    ...
    agenda._collection
        ...
        [
          ...
          },
          {
            $project: {
              ...
              queued: {
                $and: ['$nextRunAt', { $gte: [new Date(), '$nextRunAt'] }, { $gte: ['$nextRunAt', '$lastFinishedAt'] }]
              },
              completed: {
                $and: ['$lastFinishedAt', { $gt: ['$lastFinishedAt', '$failedAt'] }]
              },
      ...
      .toArray(function (err, results) {
        ...
        results.forEach(result => {
          const attachments = _.get(result, 'job.data.ticket.attachments');
          if (attachments) {
